...
from data.dataloader_sdd import TrajDataset, seq_collate
...
class Trainer:
    def __init__(self, config):
        ...
        self.traj_mean = torch.FloatTensor([0, 0]).cuda().unsqueeze(0).unsqueeze(0) #.unsqueeze(0)
        self.traj_scale = 1.
        ...
        if self.config.use_sampler:
            model_dict = torch.load('./results/{}/{}/model_{:04d}.p'.format(self.config.dataset, self.config.log, self.config.epoch), 
        ...
        self.performance = {'ADE': 10, 'FDE': 15}
    ...
    def data_preprocess(self, data):
        ...
        peds_scales = data['traj_scale'].cuda()
        past_traj_list = []
        fut_traj_list = []
    ...
    def train_single_epoch(self):
        ...
        for data in self.train_loader:
            ...
            temporal_reweight = torch.FloatTensor([(self.fut_step+1) - i for i in range(1, self.fut_step+1)]).cuda().unsqueeze(0).unsqueeze(0) / 6.
    ...
    def test_single_epoch(self):
        ...
        with torch.no_grad():
            for data in self.test_loader:
                ...
                distances = distances * proc_data_dict['peds_scales'].unsqueeze(0).unsqueeze(-1)

                ade = (distances[:, :, :]).mean(dim=-1).min(dim=0)[0].sum()
                fde = (distances[:, :, -1]).min(dim=0)[0].sum()
    ...
    def fit(self):
        ...
        for epoch in range(total_epoch):
            ...
            ade = performance['ADE'] / samples
            fde = performance['FDE'] / samples
            print('--ADE: {:.4f}\t--FDE: {:.4f}'.format(ade, fde))

            if ade < self.performance['ADE'] and fde < self.performance['FDE']:
                self.performance['ADE'] = ade
                self.performance['FDE'] = fde
                self.print_log('--best ADE: {:.4f}\t--best FDE: {:.4f}'.format(ade, fde), self.log)
    ...
    def save_data(self):
        model_dict = torch.load('./results/{}/{}/model_{:04d}.p'.format('sdd', self.config.log, self.config.epoch), 
                                map_location=torch.device('cpu'))
        self.mrf_predictor.load_state_dict(model_dict)
        ...
        with torch.no_grad():
            for data in self.test_loader:
                ...
                torch.save(save_dict, 'nmrf_pred_sample.pth')
    ...
    def test_model(self):
        ...
        print('--ADE(4.8s): {:.4f}\t--FDE(4.8s): {:.4f}'.format(performance['ADE']/samples, performance['FDE']/samples))
