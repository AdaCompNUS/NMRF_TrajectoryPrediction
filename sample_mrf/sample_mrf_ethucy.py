...
class Trainer:
    def __init__(self, config):
        ...
        self.performance = {'ADE': 10, 'FDE': 10}
        ...
        with torch.no_grad():
            for data in self.test_loader:
                ...
                if count == 4:
                    ...
                    torch.save(save_dict, 'pred_samlpes_{:04d}.pth'.format(count))
                    raise ValueError
    ...
    def test_model(self):
        ...
        with torch.no_grad():
            for data in self.test_loader:
                ...
                ade = (distances[:, :, :]).mean(dim=-1).min(dim=0)[0].sum()
                fde = (distances[:, :, -1]).min(dim=0)[0].sum()
                # ade = (distances[:, :, :]).mean(dim=-1).std(dim=0).sum()
                # fde = (distances[:, :, -1]).std(dim=0).sum()
                performance['ADE'] += ade.item()
                performance['FDE'] += fde.item()
