...
class Trainer:
    def __init__(self, config):
        ...
        self.traj_mean = torch.FloatTensor([14, 7.5]).cuda().unsqueeze(0).unsqueeze(0)
        self.traj_scale = 5.
        ...
        # load the trained vae backbone to train sampler only
        if self.config.use_sampler:
            model_dict = torch.load('./results/{}/{}/model_{:04d}.p'.format('nba', self.config.log, self.config.epoch), 
    ...
    def data_preprocess(self, data):
        """
            pre_motion_3D: torch.Size([B, 11, 10, 2]), [batch_size, num_agent, past_frame, dimension]
            fut_motion_3D: torch.Size([B, 11, 20, 2])
            seq: nba
        """
        batch_size = data['pre_motion_3D'].shape[0]
        peds_stats = torch.ones([batch_size]) * 11
    ...
    def train_single_epoch(self):
        ...
        for data in self.train_loader:
            ...
            temporal_reweight = torch.FloatTensor([(self.fut_step+1) - i for i in range(1, self.fut_step+1)]).cuda().unsqueeze(0).unsqueeze(0) / 10.
        ...
        with torch.no_grad():
            for data in self.test_loader:
                ...
                for time_i in range(1, 5):
                    ade = (distances[:, :, :5*time_i]).mean(dim=-1).min(dim=0)[0].sum()
                    fde = (distances[:, :, 5*time_i-1]).min(dim=0)[0].sum()
    ...
    def save_data(self):
        ...
        with torch.no_grad():
            for data in self.test_loader:
                ...
                # }
                save_dict = {
                    'past': data['pre_motion_3D'],
                    'future': data['fut_motion_3D'],
                    'samples': pred_mrf_sample
                }
                torch.save(save_dict, 'nba_absolute.pth')
